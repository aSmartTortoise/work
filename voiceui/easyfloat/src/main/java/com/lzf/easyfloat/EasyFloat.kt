...
class EasyFloat {
    ...
    class Builder(private val activity: Context) : OnPermissionResult {
         ...
         *  设置是否观察窗体外区域的输入事件
         */
        fun setWatchOutside(watchOutside: Boolean = false) = apply {
            config.watchOutside = watchOutside
        }

        /**
         *  设置window中的View是否消费用户的触摸事件。
         */
        fun setTouchable(touchable: Boolean = false) = apply {
            config.touchable = touchable
        }

        /**
         *  设置WindowParams type，默认为WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY
         */
