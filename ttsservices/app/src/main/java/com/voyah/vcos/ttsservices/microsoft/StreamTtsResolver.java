 ...
 * @data:2024/5/21
 **/
public class StreamTtsResolver implements IStreamTtsResolver {
    private static final String TAG = "StreamTtsResolver";
    private static final String SSML_PATTERN = "<speak xmlns='http://www.w3.org/2001/10/synthesis' xmlns:mstts='http://www.w3.org/2001/mstts' xmlns:emo='http://www.w3.org/2009/10/emotionml' version='1.0' xml:lang='language'><voice name='speaker' leadingsilence-exact=\"0ms\" style='emotionStyle' rate='5%%'>%s</voice></speak>";
...
//    private DecodeRunnable decodeRunnable;
    ...
    public StreamTtsResolver(boolean isDumpAudio, boolean isDumpLog) {
        this.dumpAudio = isDumpAudio;
        this.dumpLog = isDumpLog;
        singleThreadExecutor = Executors.newSingleThreadExecutor();
        speakingRunnable = new StreamTtsResolver.SpeakingRunnable();
...
//                    e.getAudioOffset() / 10000));
        ...
        synthesizer.SynthesisCompleted.addEventListener((o, speechSynthesisEventArgs) -> {
            ...
            String backend = speechSynthesisEventArgs.getResult().getProperties().getProperty(PropertyId.SpeechServiceResponse_SynthesisBackend);
            LogUtils.i(TAG, "SynthesisCompleted  resultId:" + resultId + " ,ttsId:" + ttsId + " ,firstByteLatencyMs:" + firstByteLatencyMs
                    + " ,underRunTimeMs:" + underRunTimeMs + " ,backend:" + backend);
            //todo:状态通知
...
//                    isParsing = false;
            ...
            if (TtsResolverManager.getInstance().isSaveMicrosoftMsg()) {
                SaveAudioUtils.saveByte(speechSynthesisEventArgs.getResult().getAudioData(), Constant.Path.BASE_PATH + "audio/" + System.currentTimeMillis());
            }
            checkSdkLogSize();
            speechSynthesisEventArgs.close();
        });
    ...
    private void startSynthesis(String ttsId, String ttsText, String styleType, String language) {
        LogUtils.i(TAG, "ttsId is " + ttsId + " ,ttsText is " + ttsText);
        //规避离线合成是单词文本过长(微软离线上限500)，当超过上限且不包含断句符号时对最后一个自负添加，处理(简单处理，不添加额外逻辑)
        ttsText = Util.replaceClosestSymbolWithPeriod(ttsText);
        //todo:开始合成状态是否应该放到begin中
        isParsing = true;
        isSynthesizing = true;
        isPlayEnd = false;
...
//        readFromLocal(ttsId, ttsText, type);
        ...
        String offLineSpeakerName = SpeakerManager.getInstance().getOffLineTtsSpeaker();
        LogUtils.d(TAG, "offLineSpeakerName:" + offLineSpeakerName + " ,currentOffLineSpeakerName:" + currentOffLineSpeakerName);
        //设置离线音色
        if (!TextUtils.equals(offLineSpeakerName, currentOffLineSpeakerName)) {
            synthesizer.getProperties().setProperty(PropertyId.SpeechServiceConnection_SynthOfflineVoice, offLineSpeakerName);
            currentOffLineSpeakerName = offLineSpeakerName;
            Log.i(TAG, "startTTS: set mic offLineSpeakerName is " + offLineSpeakerName);
        }

        String onLineSpeakerName = SpeakerManager.getInstance().getOnLineTtsSpeaker();
        String style = SsmlUtils.switchStyle(styleType, onLineSpeakerName);
        String ssmlPattern = SSML_PATTERN.replace("language", language)
                .replace("emotionStyle", style)
                .replace("speaker", onLineSpeakerName);
...
//    }
    ...
    public void beginStream(PlayTTSBean voiceTTSBean) {
        ...
        if (null != voiceTTSBean) {
            ...
            originTtsId = voiceTTSBean.getOriginTtsId();
    ...
    public synchronized void append(PlayTTSBean voiceTtsBean) {
        if (null != voiceTtsBean) {
            emotion = voiceTtsBean.getEmotion();
...
//        LogUtils.d(TAG, "isStreamBegin:" + isStreamBegin + " ,isStreamFirst:" + isStreamFirst + " ,isParsing:" + isParsing + " ,isSynthesizing:" + isSynthesizing);
        //流式请求还未发起过合成请求
        if (isStreamBegin && isStreamFirst && isCanDoFirst) {
            ...
            if (!TextUtils.isEmpty(playTts)) {
                startSynthesis(mTtsId, playTts, emotion, language);
                if (null != ttsResolverCallBack) {
            ...
            if (!TextUtils.isEmpty(appendString))
                startSynthesis(mTtsId, appendString, emotion, language);
            else if (TextUtils.isEmpty(appendString) && !isPlayEnd) {
                audioAheadProcessor.processAudioDataDelay(defaultByte, mTtsId, "append", mUsage);
                audioAheadProcessor.markAudioComplete(mTtsId, "append", mUsage);
            } else if (isPlayEnd)
