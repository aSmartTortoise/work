...
public class ReadingLightHelper {
    ...
    public static int getReadingLightStatus(int area) {
        int porpId;
        if (area == 0) {
            porpId = H56C.GW_LIN_BODY_ITL_FLREADLIGHTSTS;
        } else if (area == 1) {
            porpId = H56C.GW_LIN_BODY_ITL_FRREADLIGHTSTS;
        } else if (area == 2) {
            porpId = H56C.GW_LIN_BODY_ITL_RLREADLIGHTSTS;
        } else if (area == 3) {
            porpId = H56C.GW_LIN_BODY_ITL_RRREADLIGHTSTS;
        } else if (area == 4) {
            porpId = H56C.GW_LIN_BODY_ITL_THIRDROWLHREADLIGHTSTS;
        } else {
            porpId = H56C.GW_LIN_BODY_ITL_THIRDROWRHREADLIGHTSTS;
        }
    ...
    public static void setReadingLightState(int area, int Value) {
        if (area == VehicleArea.ROW_3_LEFT) {
            CarPropUtils.getInstance().setIntProp(H56C.IVI_BodySet3_IVI_THIRDROWLHREADLIGHTREQ, Value);
        } else if (area == VehicleArea.ROW_3_RIGHT) {
            CarPropUtils.getInstance().setIntProp(H56C.IVI_BodySet3_IVI_THIRDROWRHREADLIGHTREQ, Value);
        } else {
