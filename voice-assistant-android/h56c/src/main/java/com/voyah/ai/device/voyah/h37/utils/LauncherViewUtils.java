...
//        LogUtils.d(TAG, "isSeatPageOpen:" + isSeatPageOpen);
    ...
    public static boolean isParkOpen() {
        int intProp = CarPropUtils.getInstance().getIntProp(H56C.APA_avmState_APA_AVMSTS);
...
//    }
    ...
    public static void openParking() {
        ...
        CarPropUtils.getInstance().setIntProp(H56C.IVI_AVMSet_IVI_VOICE_AVMONOFFREQ, 1);
    }
    ...
    public static void closeParking() {
        ...
        if (intProp == 1) {
            //0=关 1=大窗 2=关 3,4=小窗
            int avmState = CarPropUtils.getInstance().getIntProp(H56C.APA_avmState_APA_VIDEODISPLAYREQ);
            if (avmState == 1) {
                // 当前为大窗显示AVM的时候，才关闭泊车应用
                CarPropUtils.getInstance().setIntProp(H56C.IVI_AVMSet_IVI_VOICE_AVMONOFFREQ, 2);
            }
        } else {
            CarPropUtils.getInstance().setIntProp(H56C.IVI_AVMSet_IVI_VOICE_APAONOFFREQ, 2);
        }
    }
