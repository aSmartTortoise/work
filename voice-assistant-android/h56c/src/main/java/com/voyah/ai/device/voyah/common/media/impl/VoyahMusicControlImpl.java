...
public enum VoyahMusicControlImpl implements VoyahMusicControlInterface {
    ...
    public void init(Context context){
        ...
        configs.add(new RemoteConfig(ServiceConfigure.MEDIAMANAGER.sourceType(), mediaManagerName, ServiceConfigure.MEDIAMANAGER.pkgName() + "." + ServiceConfigure.MEDIAMANAGER.serviceName(),  UserHandleInfo.central_screen.getUserHandle()));
        configs.add(new RemoteConfig(ServiceConfigure.MEDIAMANAGER.sourceType(), mediaManagerName, ServiceConfigure.MEDIAMANAGER.pkgName() + "." + ServiceConfigure.MEDIAMANAGER.serviceName(), UserHandleInfo.passenger_screen.getUserHandle()));
        configs.add(new RemoteConfig(ServiceConfigure.MEDIAMANAGER.sourceType(), mediaManagerName, ServiceConfigure.MEDIAMANAGER.pkgName() + "." + ServiceConfigure.MEDIAMANAGER.serviceName(),UserHandleInfo.ceil_screen.getUserHandle()));
    ...
    public MediaMusicInfo getMediaInfo(String source){
        ...
        if (mediaInfo != null) {
            if (mediaInfo.getCode() == Code.SUCESS.code()) {
                ...
                media.setSingerName(getArtist(mediaInfo.getData().getSingerName()));
                return media;
            }
        }
        return null;
        ...
        musicApi.wyCollectList(MediaConstant.TYPE_WY, request, userHandle).subscribe(new IpcObserverResponse<List<VoiceSongInfo>>() {
            ...
            public void onSuccess(List<VoiceSongInfo> response) {
                if (response != null && !response.isEmpty()) {
                    playSongs(response, response.size(),false, MediaSource.wy_music.getName());
                    for (VoiceSongInfo voiceSongInfo : response){
                        if (voiceSongInfo.isAudition() || voiceSongInfo.isCanPlay()) {
                            VoiceMusicSongInfo voiceMusicSongInfo = new VoiceMusicSongInfo();
                            voiceMusicSongInfo.setAudition(voiceSongInfo.isAudition());
                            voiceMusicSongInfo.setCanPlay(voiceSongInfo.isCanPlay());
                            voiceMusicSongInfo.setMediaName(voiceSongInfo.getMediaName());
                            voiceMusicSongInfo.setArtist(getArtist(voiceSongInfo.getArtist()));
                            onResult.onSuccess(voiceMusicSongInfo);
                            return;
                        }
                    }
        ...
        musicApi.wySearchNew(MediaConstant.TYPE_WY, request, userHandle).subscribe(new IpcObserverResponse<VoiceSearchSongInfo>(){
            ...
            public void onSuccess(VoiceSearchSongInfo response) {
                ...
                if(response != null){
                    ...
                    List<VoiceSongInfo> voiceSongInfoList = response.getVoiceSongInfoList();
                    if (voiceSongInfoList != null && !voiceSongInfoList.isEmpty()) {
                        playSongs(voiceSongInfoList,voiceSongInfoList.size(),false, MediaSource.wy_music.getName());
                        VoiceMusicSongInfo voiceMusicSongInfo = new VoiceMusicSongInfo();
                        if(response.getVoiceTips() != null){
                            if (response.getVoiceTips().getType() == Code.VOICE_SONG_CAN_NOT_PLAY.code()){
                                voiceMusicSongInfo.setCode(Code.VOICE_SONG_CAN_NOT_PLAY.code());
                                onResult.onSuccess(voiceMusicSongInfo);
                                return;
                            } else if (response.getVoiceTips().getType() == Code.VOICE_SONG_NOT_PLAY_VISIBLE.code()){
                                voiceMusicSongInfo.setCode(Code.VOICE_SONG_NOT_PLAY_VISIBLE.code());
                                onResult.onSuccess(voiceMusicSongInfo);
                                return;
                            } else if (response.getVoiceTips().getType() == Code.VOICE_SONG_NOT_FOUND.code()){
                                voiceMusicSongInfo.setCode(Code.VOICE_SONG_NOT_FOUND.code());
                                onResult.onSuccess(voiceMusicSongInfo);
                                return;
                            }
                        }
                        for (VoiceSongInfo voiceSongInfo : voiceSongInfoList){
                            if (voiceSongInfo.isAudition() || voiceSongInfo.isCanPlay()) {
                                voiceMusicSongInfo.setAudition(voiceSongInfo.isAudition());
                                voiceMusicSongInfo.setCanPlay(voiceSongInfo.isCanPlay());
                                voiceMusicSongInfo.setMediaName(voiceSongInfo.getMediaName());
                                voiceMusicSongInfo.setArtist(getArtist(voiceSongInfo.getArtist()));
                                voiceMusicSongInfo.setCode(Code.SUCESS.code());
                                onResult.onSuccess(voiceMusicSongInfo);
                                return;
                            }
                        }
    ...
    public void playCollectQq(ObserverResponse<VoiceMusicSongInfo> onResult){
        ...
        musicApi.qqCollectList(MediaConstant.TYPE_QQ, request, userHandle).subscribe(new IpcObserverResponse<List<VoiceSongInfo>>() {
            ...
            public void onSuccess(List<VoiceSongInfo> response) {
                if (response != null && !response.isEmpty()) {
                    playSongs(response, response.size(),false, MediaSource.qq_music.getName());
                    for (VoiceSongInfo voiceSongInfo : response){
                        if (voiceSongInfo.isAudition() || voiceSongInfo.isCanPlay()) {
                            VoiceMusicSongInfo voiceMusicSongInfo = new VoiceMusicSongInfo();
                            voiceMusicSongInfo.setAudition(voiceSongInfo.isAudition());
                            voiceMusicSongInfo.setCanPlay(voiceSongInfo.isCanPlay());
                            voiceMusicSongInfo.setMediaName(voiceSongInfo.getMediaName());
                            voiceMusicSongInfo.setArtist(getArtist(getArtist(voiceSongInfo.getArtist())));
                            onResult.onSuccess(voiceMusicSongInfo);
                            return;
                        }
                    }
    ...
    public void playCollectYt(ObserverResponse<VoiceMusicSongInfo> onResult){
        ...
        musicApi.ytCollectList(MediaConstant.TYPE_YT, request, userHandle).subscribe(new IpcObserverResponse<List<VoiceSongInfo>>() {
            ...
            public void onSuccess(List<VoiceSongInfo> response) {
                if (response != null && !response.isEmpty()) {
                    playSongs(response, response.size(),false, MediaSource.yt_music.getName());
                    for (VoiceSongInfo voiceSongInfo : response){
                        if (voiceSongInfo.isAudition() || voiceSongInfo.isCanPlay()) {
                            VoiceMusicSongInfo voiceMusicSongInfo = new VoiceMusicSongInfo();
                            voiceMusicSongInfo.setAudition(voiceSongInfo.isAudition());
        ...
        musicApi.ytSearch(MediaConstant.TYPE_YT, request, userHandle).subscribe(new IpcObserverResponse<List<VoiceSongInfo>>(){
            ...
            public void onSuccess(List<VoiceSongInfo> response) {
                if (response != null && !response.isEmpty()) {
                    playSongs(response, response.size(), false, MediaSource.yt_music.getName());
                    VoiceMusicSongInfo voiceMusicSongInfo = new VoiceMusicSongInfo();
                    voiceMusicSongInfo.setAudition(response.get(0).isAudition());
                    voiceMusicSongInfo.setCanPlay(response.get(0).isCanPlay());
                    voiceMusicSongInfo.setMediaName(response.get(0).getMediaName());
                    voiceMusicSongInfo.setArtist(response.get(0).getArtist());
                    onResult.onSuccess(voiceMusicSongInfo);
                    return;
                }
                onResult.onSuccess(null);
    ...
    public void playCollectXm(ObserverResponse<VoiceMusicSongInfo> onResult){
        ...
        musicApi.xmCollectList(MediaConstant.TYPE_XM, request, userHandle).subscribe(new IpcObserverResponse<List<VoiceSongInfo>>() {
            ...
            public void onSuccess(List<VoiceSongInfo> response) {
                if (response != null && !response.isEmpty()) {
                    playSongs(response, response.size(),false, MediaSource.xmly_music.getName());
                    for (VoiceSongInfo voiceSongInfo : response){
                        if (voiceSongInfo.isAudition() || voiceSongInfo.isCanPlay()) {
                            VoiceMusicSongInfo voiceMusicSongInfo = new VoiceMusicSongInfo();
                            voiceMusicSongInfo.setAudition(voiceSongInfo.isAudition());
    ...
    public void playSongs(List<VoiceSongInfo> voiceSongInfos, int voiceSongNum, boolean isAppendSong, String sourceType) {
        if (voiceSongInfos != null && !voiceSongInfos.isEmpty()) {
            List<String> ids = new ArrayList<>();
            for (int i = 0; i < voiceSongNum; i++) {
                LogUtils.d(TAG, "VoiceSongInfo: " + voiceSongInfos.get(i).toString());
                ids.add(voiceSongInfos.get(i).getMediaId());
            }
            VoicePlayRequest voicePlayRequest = new VoicePlayRequest();
            voicePlayRequest.setAppendSong(isAppendSong);
            voicePlayRequest.setPosition(0);
