...
public class SeatPropertyOperator extends Base56Operator implements IDeviceRegister {
    ...
    void init() {
        ...
        //座椅调节
        map.put(VCarSeatSignal.SEAT_ADJUST_POSITION, H56C.SCU_F_SeatControl_DSCU_SEATFRONTADJUST);
        map.put(VCarSeatSignal.SEAT_ADJUST_BACK, H56C.SCU_F_SeatControl_DSCU_SEATBACKFRONTADJUST);//靠背
        map.put(VCarSeatSignal.SEAT_ADJUST_CUSHION_HEIGHT, H56C.IVI_MemrySet1_IVI_DSSEATHEIGHTPOS1);//坐垫高度
        map.put(VCarSeatSignal.SEAT_ADJUST_CUSHION_ANGLE, H56C.IVI_MemrySet1_IVI_DSSEATCUSHIONANGLEPOS1);//坐垫角度
        map.put(VCarSeatSignal.SEAT_OCCUPIED_STATUS, H56C.ACU_state_ACU_PASSENGERSTS);//座椅占位总体状态  只读信号
        map.put(VCarSeatSignal.SEAT_WELCOME_MODE_SWITCH, Comforts.ID_SEAT_CTRL_WELCOME_MODE_SETTING);
        map.put(VCarSeatSignal.SEAT_MIRROR_SAVE, H56C.IVI_SCUCtrl_IVI_DSMSETMEMORYREQ);//后视镜记忆存储
        map.put(VCarSeatSignal.SEAT_MIRROR_RECOVERY, H56C.IVI_SCUCtrl_IVI_DSMGETMEMORYREQ);//后视镜记忆调取
    ...
    public void setBaseIntProp(String key, int area, int value) {
        ...
        switch (key) {
            case VCarSeatSignal.SEAT_HEAT_CUR_LEVEL:
                ...
                if (area_37 == VehicleArea.ROW_3_LEFT || area_37 == VehicleArea.ROW_3_RIGHT) {
                    // 三排座椅只有开关
                    value = value == 0 ? 1 :2;
                }
                break;
        ...
        switch (key) {
            ...
            case VCarSeatSignal.SEAT_PAGE_SATE:
                return SeatServiceImpl.getInstance(Utils.getApp())
                        .isCurrentState(area == 0 ? "ACTION_GET_SEAT_PANEL_STATE" : "ACTION_GET_SEAT_PANEL_STATE_displayId2");
