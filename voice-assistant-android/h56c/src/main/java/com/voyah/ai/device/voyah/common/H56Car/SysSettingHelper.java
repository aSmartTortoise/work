...
public class SysSettingHelper {
    ...
    public static boolean isChargeEnable(int area) {
        int workingStatesFL = CarPropUtils.getInstance().getIntProp(H56C.WCM_state_WCM_WORKINGSTS);
        int workingStatesFR = CarPropUtils.getInstance().getIntProp(H56C.WCM_FR_state_WCM_FR_WORKINGSTS);
        int workingStatesRL = CarPropUtils.getInstance().getIntProp(H56C.WCM_RL_state_WCM_RL_WORKINGSTS);
        int workingStatesRR = CarPropUtils.getInstance().getIntProp(H56C.WCM_RR_state_WCM_RR_WORKINGSTS);
    ...
    public static void setChargeSwitchState(int area, int value) {
        switch (area) {
            case 0:
                CarPropUtils.getInstance().setIntProp(H56C.IVI_BodySet4_IVI_WCMFLONOFFSET, value);
                break;
            case 1:
                CarPropUtils.getInstance().setIntProp(H56C.IVI_BodySet4_IVI_WCMFRONOFFSET, value);
                break;
            case 2:
                CarPropUtils.getInstance().setIntProp(H56C.IVI_BodySet4_IVI_WCMRLONOFFSET, value);
                break;
            case 3:
                CarPropUtils.getInstance().setIntProp(H56C.IVI_BodySet4_IVI_WCMRRONOFFSET, value);
                break;
        }
    ...
    public static int getChargeSwitchState(int area) {
        switch (area) {
            case 0:
                CarPropUtils.getInstance().getIntProp(H56C.WCM_state_WCM_WORKINGSTS);
                break;
            case 1:
                CarPropUtils.getInstance().getIntProp(H56C.WCM_FR_state_WCM_FR_WORKINGSTS);
                break;
            case 2:
                CarPropUtils.getInstance().getIntProp(H56C.WCM_RL_state_WCM_RL_WORKINGSTS);
                break;
            case 3:
                CarPropUtils.getInstance().getIntProp(H56C.WCM_RR_state_WCM_RR_WORKINGSTS);
                break;
        }
        return 0;
    }
    ...
    public static boolean getVolumeWithSpeedStatus() {
        return CarPropUtils.getInstance().getIntProp(Infotainment.ID_AMP_VEHICLESPEEDCOMP_MODEENABLE) == 1;
    }

    public static void setVolumeFollowSpeed(boolean onOff) {
        Settings.System.putInt(Utils.getApp().getContentResolver(), "key_vehicle_speedcomp", onOff ? 2 : 1);
        CarPropUtils.getInstance().setIntProp(Infotainment.ID_AMP_VEHICLESPEEDCOMP_MODEENABLE, onOff ? 2 : 1);
    }
    ...
    public static int getDrvAssistBroadcastState() {
        try {
            return IFunctionManagerImpl.getInstance(Utils.getApp()).getDriveAidBroadcast();
        } catch (RemoteException e) {
    ...
    public static void setDrvAssistBroadcastState(int onOff) {
        try {
            IFunctionManagerImpl.getInstance(Utils.getApp()).setDriveAidBroadcast(onOff > 0);
        } catch (RemoteException e) {
            //
        }
    }
    ...
    public static boolean getLowSpeedPedesWarningSwitchState() throws Settings.SettingNotFoundException {
        ...
        if (carModeConfig == 2) {
            //N2车型
            int value = CarPropUtils.getInstance().getIntProp(H56C.AMP_state_AMP_VSP_SOUNDENABLE);
            return value > 0;
        } else {
            //N3N4车型
//            int value = CarPropUtils.getInstance().getIntProp(H56C.AMP_Response1_AMP_VSP_SOUNDENABLE2);
            //字段默认值是2 = 科技未来
            int value = Settings.System.getInt(Utils.getApp().getContentResolver(), "sound_low_speed_warning_switch", 2);
            return value == 2;
        }
    }
    ...
    public static void setLowSpeedPedesWarningState(boolean state) {
        int carModeConfig = CarServicePropUtils.getInstance().getCarModeConfig();
        if (carModeConfig == 2) {
            CarPropUtils.getInstance().setIntProp(H56C.IVI_infoSet_IVI_VSPONOFFSET, state ? 2 : 1);
        } else {
            int soundLowSpeedWarning = Settings.System.getInt(Utils.getApp().getContentResolver(), "sound_low_speed_warning", 2);
            CarPropUtils.getInstance().setIntProp(H56C.IVI_infoSet1_IVI_VSPONOFFSET2, state ? soundLowSpeedWarning : 1);
            Settings.System.putInt(Utils.getApp().getContentResolver(), "sound_low_speed_warning_switch", state ? 2 : 1);
        }
    }
    ...
    public static void setLowSpeedPedesWarningMode(int mode) throws Settings.SettingNotFoundException {
        CarPropUtils.getInstance().setIntProp(H56C.IVI_infoSet1_IVI_VSPONOFFSET2, mode);
    ...
    public static void setHeadrestSoundState(int value) {
        try {
            IFunctionManagerImpl.getInstance(Utils.getApp()).setHeadrestAudioMode(value);
        } catch (RemoteException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public static int getSoundEffectsMode() {
        return Settings.System.getInt(Utils.getApp().getContentResolver(), "sound_effect_mode", 1);
    }

    public static void setSoundEffectsMode(int value) {
        Settings.System.putInt(Utils.getApp().getContentResolver(), "sound_effect_mode", value);
        CarPropUtils.getInstance().setIntProp(H56C.IVI_infoSet1_IVI_VIRTUALSOUNDFIELDSET, value);
    }

    public static int getVolumeFocusMode() {
        int keyFocusSound = Settings.System.getInt(Utils.getApp().getContentResolver(), "key_focus_sound", 1);
        return keyFocusSound;
    }

    public static void setVolumeFocusMode(int value) {
        Settings.System.putInt(Utils.getApp().getContentResolver(), "key_focus_sound", value);
    }
