...
public class DnnUtils {
    ...
    private static final String DNN1_SUFFIX2 = "106.30";
    private static final String DNN2_SUFFIX2 = "103.30";
    private static final String DNN3_SUFFIX2 = "107.30";
    private static final String DNN4_SUFFIX2 = "108.30";


    public static void switchToAllNetwork(Context context, boolean enable) {
        LogUtils.i(TAG, "switchToAllNetwork() called with: enable = [" + enable + "]");
        ConnectivityManager connMgr = (ConnectivityManager) context.getSystemService(Context.CONNECTIVITY_SERVICE);
        if (!enable) {
            Network[] networks = connMgr.getAllNetworks();
            LogUtils.i(TAG, "networks length:" + networks.length);
            for (Network network : networks) {
                ...
                if (lp != null) {
                    for (LinkAddress linkAddress : lp.getLinkAddresses()) {
                        ...
                        LogUtils.v(TAG, "address:" + address + "," + network);
                        if (address.endsWith(DNN1_SUFFIX) || address.endsWith(DNN1_SUFFIX2)) {
                            privateNetWorkDnn1Id = Integer.parseInt(network.toString());
                            privateNetWorkDnn1 = getPrivateDnsNet(network);
                        }
                        if (address.endsWith(DNN2_SUFFIX) || address.endsWith(DNN2_SUFFIX2)) {
                            publicNetWorkDnn2Id = Integer.parseInt(network.toString());
                            publicNetWorkDnn2 = getPrivateDnsNet(network);
                        }
                        if (address.endsWith(DNN3_SUFFIX) || address.endsWith(DNN3_SUFFIX2)) {
                            ...
                            if (!dnsServers.isEmpty()) {
                                for (int i = 0; i < dnsServers.size(); i++) {
                                    LogUtils.v(TAG, "dnsServers[" + i + "]:" + dnsServers.get(i).toString());
                                }
                            } else {
                                LogUtils.i(TAG, "dnsServers is empty");
                            }
                        }
                        if (address.endsWith(DNN4_SUFFIX) || address.endsWith(DNN4_SUFFIX2)) {
            ...
            LogUtils.i(TAG, "unbindProcessToNetwork ret:" + ret);
        }
    }
    ...
    private static void bindToNetwork(Network network, ConnectivityManager connMgr) {
        ...
        LogUtils.i(TAG, "bindProcessToNetwork ret:" + result);
    }
}
