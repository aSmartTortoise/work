...
@ClassAgent
public class MediaTextSwitchAgent extends BaseAgentX {

    private static final String TAG = MediaTextSwitchAgent.class.getSimpleName();

    @Override
    public String AgentName() {
        return "media_text#switch";
    }

    @Override
    public ClientAgentResponse executeAgent(Map<String, Object> flowContext, Map<String, List<Object>> paramsMap) {
        LogUtils.i(TAG, "-----run--MediaTextSwitchAgent------");
        ...
        TTSBean tts = DeviceHolder.INS().getDevices().getMedia().switchLyric(isOpen);
