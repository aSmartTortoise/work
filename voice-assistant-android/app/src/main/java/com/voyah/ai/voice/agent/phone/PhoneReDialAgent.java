 ...
 **/
@ClassAgent
public class PhoneReDialAgent extends BaseAgentX {
    private static final String TAG = PhoneReDialAgent.class.getSimpleName();
    ...
    public ClientAgentResponse executeAgent(Map<String, Object> flowContext, Map<String, List<Object>> paramsMap) {
        LogUtils.i(TAG, "---------PhoneReDialAgent----------");
        ...
        //3.无法定位到最近一个去电
        lastOutgoingNumber = phoneInterface.getLastOutgoingNumber();
        LogUtils.i(TAG, " lastOutgoingNumber is " + lastOutgoingNumber);
        if (StringUtils.isBlank(lastOutgoingNumber)) {
            ttsBean = PhoneUtils.getTtsBean(TTSAnsConstant.PHONE_FIND_FAIL_NUMBER[0], TTSAnsConstant.PHONE_FIND_FAIL_NUMBER[1]);
            return new ClientAgentResponse(Constant.PhoneAgentResponseCode.BT_STATE_NOT_SATISFY, flowContext, ttsBean);
        } else {
            LogUtils.i(TAG, "即将呼叫" + lastOutgoingNumber);
            //todo:TTS播报 即将呼叫XXX
            ttsBean = PhoneUtils.getTtsBean(TTSAnsConstant.PHONE_REDIAL[0], TTSAnsConstant.PHONE_REDIAL[1]);
            PhoneUtils.getReplacesTtsBean(ttsBean, new String[]{"@{tel_name}", "@{tel_number}"}, new String[]{"", lastOutgoingNumber});
    ...
    public void executeOrder(String executeTag, int location) {
        if (StringUtils.equals(executeTag, TTSAnsConstant.PHONE_REDIAL[0]))
            phoneInterface.placeCall(lastOutgoingNumber);
    }
