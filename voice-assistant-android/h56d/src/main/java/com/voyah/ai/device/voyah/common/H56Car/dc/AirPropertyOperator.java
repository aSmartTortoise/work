...
public class AirPropertyOperator extends Base56Operator implements IDeviceRegister {
    ...
    void init() {
        ...
        //todo 循环模式调节， 56D只有内外循环判断，和设置。
        map.put(AirSignal.AIR_CYCLE_MODE, H56D.IVI_BodySet3_IVI_HVACRECIRCMODESET);
        //todo 自动循环模式开关,
        map.put(AirSignal.AIR_CYCLE_AUTO_STATE, H56D.AC_Cfg_Rsp_AC_AUTOCIRCULONOFF_STS);
        //todo 调节空调工作模式，新功能。
//        map.put(, Climate.ID_COMFORT_CONFIG);

        //干燥模式 修改
        map.put(AirSignal.DRY_MODE, H56D.AC_Cfg_Rsp_AC_ACDORMODE_STS);
        //干燥除味，开关状态。修改是另外的信号
        map.put(AirSignal.DRY_MODE_SWITCH_STATE, H56D.AC_Cfg_Rsp_AC_ACDRYINGANDODORREMOVAL_STS);

        //通风降温开关  修改
        map.put(AirSignal.VENTILATION_SWITCH_STATE, H56D.AC_Cfg_Rsp_AC_VENTILATIONCOOL_STS);
//        自动除霜开关  修改
        map.put(AirSignal.AUTO_DEFROST_SWITCH_STATE, H56D.AC_Cfg_Rsp_AC_AUTODEFONOFF_STS);
        //自动除霜等级  修改
        map.put(AirSignal.AUTO_DEFROST_LEVEL, H56D.AC_Cfg_Rsp_AC_AUTODEFLEVEL_STS);
        //auto开关
        ...
        //智能开关
        map.put(AirSignal.AIR_INTELLIGENT_IDENTIFICATION_SWITCH_STATE, H56D.AC_AirQuality_AC_BACKROWACAUTOSTA);
        //座舱温度
        map.put(AirSignal.AIR_CABIN_TEMP, H56D.AC2_AC_CABINTEMP);
        //快速制冷开关
        map.put(AirSignal.AIR_FAST_COOLING_SWITCH_STATE, H56D.AC_Ctrl_F_Rsp_AC_RAPIDCOOL_STS);
        //速热开关
        map.put(AirSignal.AIR_FAST_HEATER_SWITCH_STATE, H56D.AC_Ctrl_F_Rsp_AC_RAPIDHEAT_STS);
    }
...
//0x6:FACE_AND_FOOT_AND_DEFROST
    ...
    public int getBaseIntProp(String key, int area) {
        if (key == CommonSignal.COMMON_GEAR_INFO) {
            //56D的R挡判断比较特殊。
            boolean isParking = DeviceHolder.INS().getDevices().getVoiceCarSignal().isParkingLimitation();
            return isParking ? 1 : 0;
        }
        ...
        if (curKey == null) {
            switch (key) {
                ...
                case AirSignal.MAX_AIR_WIND:
                    if (area == PositionSignal.FIRST_ROW_LEFT || area == PositionSignal.FIRST_ROW_RIGHT || area == PositionSignal.FIRST_ROW) {
                        curValue = ParamsFrontBlwLevel.LEVEL7 + 1;
                    } else {
                        curValue = ParamsFrontBlwLevel.LEVEL5 + 1;
                    }
                    break;
                case AirSignal.MIN_AIR_WIND:
                    curValue = ParamsFrontBlwLevel.LEVEL1 + 1;
                    break;
                default:
                    curValue = -1;
            ...
            curValue = carPropHelper.getIntProp(curKey,
                    realPosition);
            if (key.equals(AirSignal.AIR_WIND)) {
                curValue++;
            }
        }
...
//                    String[] occupyStatusValue = array.split(",");
            String str = "";
            int rightSTS1 = carPropHelper.getIntProp(H56D.ACU_state_ACU_PASSENGERSTS);
            int leftSTS2 = carPropHelper.getIntProp(H56D.ACU_state_ACU_SECONDLEFTSTS);
            int rightSTS2 = carPropHelper.getIntProp(H56D.ACU_state_ACU_SECONDRIGHTSTS);
            int leftSTS3 = carPropHelper.getIntProp(H56D.BCM_state3_BCM_3NDLEFTSTS);
            int rightSTS3 = carPropHelper.getIntProp(H56D.BCM_state3_BCM_3NDRIGHTSTS);
...
//                0x1：内循环 0x2：外循环 0x3：自动循环
                curKey = H56D.IVI_BodySet3_IVI_HVACRECIRCMODESET;
                break;
            case AirSignal.DRY_MODE:
                curKey = H56D.AC_Cfg_Cmd_IVI_ACDORMODE_REQ;
                curValue = AIR_DRY_MODE_STRING_INTEGER_MAP.get(value) + 1;
                break;
            case AirSignal.AUTO_DEFROST_LEVEL:
                curKey = H56D.AC_Cfg_Cmd_IVI_AUTODEFLEVEL_REQ;
    ...
    public void setBaseBooleanProp(String key, int area, boolean value) {
        ...
        switch (key) {
            ...
            case AirSignal.VENTILATION_SWITCH_STATE:
                realKey = H56D.AC_Cfg_Cmd_IVI_VENTILATIONCOOL_REQ;
                realBoolean = value ? 2 : 1;
                break;
            case AirSignal.AIR_INTELLIGENT_IDENTIFICATION_SWITCH_STATE:
                realKey = H56D.IVI_ACHCMCtrl_IVI_BACKROWACAUTOSET;
                realBoolean = value ? 1 : 2;
                break;
            case AirSignal.AIR_CYCLE_AUTO_STATE:
                //打开自动循环
                realKey = H56D.IVI_BodySet3_IVI_HVACRECIRCMODESET;
                if (value) {
                    realBoolean = 3;
                } else {
                    realBoolean = 1;
            ...
            case AirSignal.AUTO_DEFROST_SWITCH_STATE:
                //自动除霜功能开关。
                realKey = H56D.AC_Cfg_Cmd_IVI_AUTODEFONOFF_REQ;
                realBoolean = value ? 2 : 1;
                break;
            default:
                realKey = map.get(key);
