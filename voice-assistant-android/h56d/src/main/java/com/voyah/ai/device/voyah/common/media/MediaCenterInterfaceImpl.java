...
//                    }
    ...
    public TTSBean pre() {
        ...
        if (!TextUtils.isEmpty(source)) {
            //TODO 4/27 调用媒体中心无效
            if (VideoControlCenter.getInstance().isVideoSource(voyahMusicControlInterface.getSource())) {
    ...
    public boolean switchVideoPage(String uiName, String appName, boolean isOpen, String mediaType) {
        String pkgName = MediaHelper.getPackageName(appName);
        LogUtils.d(TAG, "switchVideoPage pkgName = " + pkgName + ", uiName = " + uiName + ", displayId = " + displayId + ", mediaType = " + mediaType);
    ...
    public Map<String, String> uploadPlayStatus() {
        ...
        if (isPlayingAllScreen()) {
            ...
            isPlaying = true;
        } else if (VideoControlCenter.getInstance().isVideoStatus("", VideoControlCenter.MediaStatus.PLAYING)) {
            mediaSource = MediaSource.translateMediasource(VideoControlCenter.getInstance().getCurrentSource()).getType();
            isPlaying = true;
        } else {
            mediaSource = "";
            isPlaying = false;
