...
import com.voyah.cockpit.appadapter.aidlimpl.LauncherCeilingScreenSaverServiceImpl;
...
import com.voyah.cockpit.launcher.ILauncherCommonCallBack;
...
public class LauncherManager implements LauncherInterface {
    ...
    public static final String VCOS_NOTIFICATION_SHOW = "VCOS_NOTIFICATION_SHOW"; //显示隐藏通知中心KEY

    public static final String VCOS_NOTIFICATION_MESSAGE = "VCOS_NOTIFICATION_MESSAGE"; //获取消息状态KEY
    public static final String VCOS_NOTIFICATION_CANCELALL = "VCOS_NOTIFICATION_CANCELALL"; //清空通知中心消息KEY
    ...
    private LauncherCeilingScreenSaverServiceImpl launcherCeilingScreenSaverService; //吸顶屏屏保

    private static LauncherManager launcherManager;

    private boolean isDriveDesktop; // 是否行车桌面
    ...
    public void init() {
        ...
        launcherCommonService.startService(() -> {
            LogUtils.i(TAG, "launcherCommonService onServiceConnected");
            launcherCommonService.registerListener(listener);
            launcherCommonService.isDriveDesktop(result -> isDriveDesktop = result);
        });
        launcherCommonService.moveAllFreeFormTaskToBack();
        ...
        launcherSecondaryScreenSaverService.startService(() -> LogUtils.i(TAG, "launcherSecondaryScreenSaverService onServiceConnected"));
        launcherCeilingScreenSaverService = LauncherCeilingScreenSaverServiceImpl.getInstance(Utils.getApp());
        launcherCeilingScreenSaverService.startService(() -> LogUtils.i(TAG, "launcherCeilingScreenSaverService onServiceConnected"));
    }
    ...
    private Wallpaper getCurrentWallpaper() {
        ...
        wallpaperService.getCurrentWallpaper(result -> {
            wallpaper[0] = result;
    ...
    public void backToHome(DeviceScreenType screenType) {
        {
            ...
            if (screenType == DeviceScreenType.CEIL_SCREEN) {
                SystemUiUtils.getInstance().backHome(3000);
...
//                Utils.getApp().startActivity(it);
                ...
                SystemUiUtils.getInstance().backHome(2000);
//                Intent it = new Intent();
...
//                Utils.getApp().startActivity(it);
                ...
                SystemUiUtils.getInstance().backHome(1000);
//                DeviceHolder.INS().getDevices().getSystem().getApp().openApp(MultiAppHelper.LAUNCHER_PKG, DeviceScreenType.CENTRAL_SCREEN);
    ...
    private final ILauncherCommonCallBack.Stub listener = new ILauncherCommonCallBack.Stub() {
        @Override
        public void onDriveDesktopMode(boolean state) throws RemoteException {
            LogUtils.d(TAG, "onDriveDesktopMode() called with: state = [" + state + "]");
            isDriveDesktop = state;
            DeviceHolder.INS().getDevices().getSystem().getSplitScreen().onSplitStatusChanged(state);
        }

        @Override
        public void onSwitchScreenResult(int state) throws RemoteException {
        }

        @Override
        public void onDriveDesktopAnimate(int animate, int gear) throws RemoteException {
        }

        @Override
        public void onShowAdas(boolean show) throws RemoteException {
        }
    };
    ...
    public boolean isAllAppPanelShowing() {
        ...
        CountDownLatch countDownLatch = new CountDownLatch(1);
        launcherCommonService.isAllAppPanelShowing(result -> {
    ...
    public boolean isSecondAllAppPanelShowing() {
        ...
        CountDownLatch countDownLatch = new CountDownLatch(1);
        launcherSecondaryService.isShowingAppListOfSecondary(result -> {
    ...
    public boolean isCeilAllAppPanelShowing() {
        ...
        CountDownLatch countDownLatch = new CountDownLatch(1);
        launcherCeilingService.isShowingAppListOfSecondary(result -> {
    ...
    public void showHideScreenSaver(boolean isShow, ResultCallBack<Boolean> callBack) {
        launcherSecondaryScreenSaverService.showHideScreenSaver(isShow, callBack::onResult);
    }

    @Override
    public void showHideScreenSaver(boolean isShow) {
        launcherSecondaryScreenSaverService.showHideScreenSaver(isShow,
                result -> LogUtils.d(TAG, "showHideScreenSaver result:" + result));
    }

    @Override
    public void isShowingScreenSaverOfSecondary(ResultCallBack<Boolean> callBack) {
        launcherSecondaryScreenSaverService.isShowingScreenSaverOfSecondary(callBack::onResult);
    }

    @Override
    public boolean isShowingScreenSaverOfSecondary() {
        final boolean[] isShowingScreenSaverOfSecondary = {false};
        launcherSecondaryScreenSaverService.isShowingScreenSaverOfSecondary(result -> {
    ...
    public void showHideCeilScreenSaver(boolean isShow, ResultCallBack<Boolean> callBack) {
        launcherCeilingScreenSaverService.showHideScreenSaver(isShow, callBack::onResult);
    }

    @Override
    public void showHideCeilScreenSaver(boolean isShow) {
        launcherCeilingScreenSaverService.showHideScreenSaver(isShow,
                result -> LogUtils.d(TAG, "showHideCeilScreenSaver result:" + result));
    }

    @Override
    public void isShowingScreenSaverOfCeil(ResultCallBack<Boolean> callBack) {
        launcherCeilingScreenSaverService.isShowingScreenSaverOfCeiling(callBack::onResult);
    }

    @Override
    public boolean isShowingScreenSaverOfCeil() {
        final boolean[] isShowingScreenSaverOfSecondary = {false};
        launcherCeilingScreenSaverService.isShowingScreenSaverOfCeiling(result -> {
            LogUtils.d(TAG, "isShowingCeilScreenSaverOfSecondary  result:" + result);
            isShowingScreenSaverOfSecondary[0] = result;
        });
        return isShowingScreenSaverOfSecondary[0];
    }
    ...
    public boolean isNegativeScreenOpen(DeviceScreenType deviceScreenType) {
        return SystemUiUtils.getInstance().isNegativeScreenOpen(deviceScreenType.name());
    }

    @Override
    public void openNegativeScreen(DeviceScreenType deviceScreenType) {
        SystemUiUtils.getInstance().getNegativeScreenState(true, deviceScreenType.name());
    }


    @Override
    public void closeNegativeScreen(DeviceScreenType deviceScreenType) {
        SystemUiUtils.getInstance().getNegativeScreenState(false, deviceScreenType.name());
    }
    ...
    public boolean isDriveDesktop() {
        return isDriveDesktop;
    }

    @Override
    public boolean isGearsR() {
        int gears = DeviceHolder.INS().getDevices().getCarServiceProp().getIntProp(Driving.ID_DRV_INFO_GEAR_POSITION);
        LogUtils.d(TAG, "isGearsRForbidden gears:" + gears);
        return gears == 1;
    }
    ...
    public boolean isGearsROrD() {
        int gears = DeviceHolder.INS().getDevices().getCarServiceProp().getIntProp(Driving.ID_DRV_INFO_GEAR_POSITION);
        LogUtils.d(TAG, "isGearsROrD gears:" + gears);
        return gears == 1 || gears == 3;
    }
    ...
    public boolean isScreenViewMode(DeviceScreenType deviceScreenType) {
        return DeviceHolder.INS().getDevices().getMediaCenter().getVideoPlayingByDisplayId(deviceScreenType);
    }
    ...
    public boolean isMessageCenterOpen() {
        ...
        int type = Settings.Secure.getInt(contentResolver, VCOS_NOTIFICATION_SHOW, 1);
        LogUtils.d(TAG, "isMessageCenterOpen:type:" + type);
        return type == 1;
    }

    @Override
    public void switchMessageCenter(boolean switchType) {
        LogUtils.d(TAG, "switchMessageCenter switchType:" + switchType);
        Uri uri = Settings.Secure.getUriFor(VCOS_NOTIFICATION_SHOW);
        ContentResolver contentResolver = Utils.getApp().getContentResolver();
        Settings.Secure.putInt(contentResolver, VCOS_NOTIFICATION_SHOW, switchType ? 1 : 0);
    ...
    public boolean isMessageCenterNotEmpty() {
        ...
        String jsonString = Settings.Secure.getString(Utils.getApp().getContentResolver(),
                VCOS_NOTIFICATION_MESSAGE);
        LogUtils.d(TAG, "isMessageCenterNotEmpty jsonString:" + jsonString);
        if (StringUtils.isBlank(jsonString))
            return false;
        try {
            JSONObject jsonObject = GsonUtils.parseToJSONObject(jsonString);
            hasUnread = jsonObject.getBoolean("hasUnread"); //有无未读消息
//            jsonObject.getBoolean("hasWarn"); //有无故障提醒的消息(故障提醒消息无法清空，只有解除故障后才会自动消失)
...
//            jsonObject.getInt("from");
    ...
    public void clearMessageCenter() {
        LogUtils.d(TAG, "clearMessageCenter");
        Uri uri = Settings.Secure.getUriFor(VCOS_NOTIFICATION_CANCELALL);
        ContentResolver contentResolver = Utils.getApp().getContentResolver();
        Settings.Secure.putInt(contentResolver, VCOS_NOTIFICATION_CANCELALL, 1);
        contentResolver.notifyChange(uri, null);
    }

    @Override
    public boolean isRcwOpen() {
        int rcWFunctionStatus = DeviceHolder.INS().getDevices().getCarServiceProp().getIntProp(H56D.ADS_FunctionStatus_ADS_RCWFUNCTIONSTATUS);
        LogUtils.d(TAG, "isRcwOpen rcWFunctionStatus:" + rcWFunctionStatus);
        return rcWFunctionStatus == 3;
    }
