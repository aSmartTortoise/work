...
public class FuelportPropertyOperator extends Base56Operator {
    @Override
    void init() {
        map.put(FuelPortSignal.FUEL_PORT_SWITCH, EntryLocks.ID_FUEL_PORT);
        map.put(FuelPortSignal.FUEL_PRESSURE_RELIEFSTA, H56D.BCM_state4_BCM_PRESSURERELIEFSTA);
    }
    ...
    public boolean getBaseBooleanProp(String key, int area) {
        if (key.equalsIgnoreCase(FuelPortSignal.FUEL_PORT_SWITCH)) {
            return CarPropUtils.getInstance().getIntProp(EntryLocks.ID_FUEL_PORT) == 1;
        } else {
    ...
    public void setBaseBooleanProp(String key, int area, boolean value) {
        if (key.equalsIgnoreCase(FuelPortSignal.FUEL_PORT_SWITCH)) {
            CarPropUtils.getInstance().setIntProp(EntryLocks.ID_FUEL_PORT, value ? 1 : 2);
        } else {
