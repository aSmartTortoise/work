...
public class SystemSettingPropertyOperator extends Base56Operator implements IDeviceRegister {
    ...
    @Override
    void init() {
        map.put(SysSettingSignal.SYS_WIRELESS_CHARGE, H56D.IVI_BodySet4_IVI_WCMFLONOFFSET); //映射只用来写，读有特殊处理
        //map.put(SysSettingSignal.SYS_VOLUME_FOLLOW_SPEED, Infotainment.ID_AMP_VEHICLESPEEDCOMP_MODEENABLE); //只用来读，写要走设置接口，否则设置不刷新UI
        map.put(SysSettingSignal.SYS_ACOUSTIC_VEH_ALERT_SWITCH, H56D.IVI_infoSet1_IVI_VSPONOFFSET2); //低速行人报警音
...
//        map.put(SysSettingSignal.SYS_5GP_SWITCH, Signal.ID_TBOX5GONOFFSET);
        ...
        map.put(SysSettingSignal.SYS_SOUND_EFFECTS_MODE, H56D.IVI_infoSet1_IVI_VIRTUALSOUNDFIELDSET);
        map.put(SysSettingSignal.SYS_VOLUME_FEATURE_MODE_N4, H56D.IVI_infoSet1_IVI_AI_MODESET);
...
//        init("8295and", "com.voyah.ai.voice", getAppVersion());
    }
...
//    public native int init(String ecu, String appName, String ver);
    ...
    public boolean getBaseBooleanProp(String key, int area) {
        switch (key) {
            ...
            case SysSettingSignal.SYS_AUTO_PARKING_HINT_SWITCH:
                return SysSettingHelper.getAutoParkingHintState();
    ...
    public void setBaseBooleanProp(String key, int area, boolean value) {
        switch (key) {
            ...
            case SysSettingSignal.SYS_INTELLIGENT_VOLUME_SWITCH:
                ...
                break;
            case SysSettingSignal.SYS_AUTO_PARKING_HINT_SWITCH:
                SysSettingHelper.setAutoParkingHintState(value);
                break;
            case SysSettingSignal.SYS_REAR_BELT_REMINDER_SWITCH:
            default:
