...
import com.voice.sdk.device.media.PlayStateCallBack;
...
import com.voyah.media.common.model.mediamanager.CmdTarget;
...
public enum VoyahMusicControlImpl implements VoyahMusicControlInterface {
    ...
    private final CmdTarget cmdTarget = new CmdTarget();
    ...
    public void init(Context context){
        ...
        // 播控等公共功能
        ComponentName mediaManagerName = new ComponentName(MediaConstant.MEDIACENTER_APP_NAME, ServiceConfigure.MEDIAMANAGER.pkgName() + ".service." + ServiceConfigure.MEDIAMANAGER.serviceName());
        configs.add(new RemoteConfig(ServiceConfigure.MEDIAMANAGER.sourceType(), mediaManagerName, ServiceConfigure.MEDIAMANAGER.pkgName() + "." + ServiceConfigure.MEDIAMANAGER.serviceName()));
        ...
        configs.add(new RemoteConfig(ServiceConfigure.UI.sourceType(), uiName, ServiceConfigure.UI.pkgName() + "." + ServiceConfigure.UI.serviceName(), UserHandleInfo.central_screen.getUserHandle()));
        configs.add(new RemoteConfig(ServiceConfigure.UI.sourceType(), uiName, ServiceConfigure.UI.pkgName() + "." + ServiceConfigure.UI.serviceName(), UserHandleInfo.passenger_screen.getUserHandle()));
        configs.add(new RemoteConfig(ServiceConfigure.UI.sourceType(), uiName, ServiceConfigure.UI.pkgName() + "." + ServiceConfigure.UI.serviceName(), UserHandleInfo.ceil_screen.getUserHandle()));
        ...
        configs.add(new RemoteConfig(ServiceConfigure.YTMUSIC.sourceType(), ytMusicName, ServiceConfigure.YTMUSIC.pkgName() + "." + ServiceConfigure.YTMUSIC.serviceName(), UserHandleInfo.central_screen.getUserHandle()));
        configs.add(new RemoteConfig(ServiceConfigure.YTMUSIC.sourceType(), ytMusicName, ServiceConfigure.YTMUSIC.pkgName() + "." + ServiceConfigure.YTMUSIC.serviceName(), UserHandleInfo.passenger_screen.getUserHandle()));
        configs.add(new RemoteConfig(ServiceConfigure.YTMUSIC.sourceType(), ytMusicName, ServiceConfigure.YTMUSIC.pkgName() + "." + ServiceConfigure.YTMUSIC.serviceName(), UserHandleInfo.ceil_screen.getUserHandle()));
        ...
        configs.add(new RemoteConfig(ServiceConfigure.XMLYMUSIC.sourceType(), xmMusicName, ServiceConfigure.XMLYMUSIC.pkgName() + "." + ServiceConfigure.XMLYMUSIC.serviceName(), UserHandleInfo.central_screen.getUserHandle()));
        configs.add(new RemoteConfig(ServiceConfigure.XMLYMUSIC.sourceType(), xmMusicName, ServiceConfigure.XMLYMUSIC.pkgName() + "." + ServiceConfigure.XMLYMUSIC.serviceName(),UserHandleInfo.passenger_screen.getUserHandle()));
        configs.add(new RemoteConfig(ServiceConfigure.XMLYMUSIC.sourceType(), xmMusicName, ServiceConfigure.XMLYMUSIC.pkgName() + "." + ServiceConfigure.XMLYMUSIC.serviceName(), UserHandleInfo.ceil_screen.getUserHandle()));
        ...
        configs.add(new RemoteConfig(ServiceConfigure.QQMUSIC.sourceType(), qqMusicName, ServiceConfigure.QQMUSIC.pkgName() + "." + ServiceConfigure.QQMUSIC.serviceName(), UserHandleInfo.central_screen.getUserHandle()));
        configs.add(new RemoteConfig(ServiceConfigure.QQMUSIC.sourceType(), qqMusicName, ServiceConfigure.QQMUSIC.pkgName() + "." + ServiceConfigure.QQMUSIC.serviceName(), UserHandleInfo.passenger_screen.getUserHandle()));
        configs.add(new RemoteConfig(ServiceConfigure.QQMUSIC.sourceType(), qqMusicName, ServiceConfigure.QQMUSIC.pkgName() + "." + ServiceConfigure.QQMUSIC.serviceName(), UserHandleInfo.ceil_screen.getUserHandle()));
        ...
        configs.add(new RemoteConfig(ServiceConfigure.WYMUSIC.sourceType(), wyMusicName, ServiceConfigure.WYMUSIC.pkgName() + "." + ServiceConfigure.WYMUSIC.serviceName(), UserHandleInfo.central_screen.getUserHandle()));
        configs.add(new RemoteConfig(ServiceConfigure.WYMUSIC.sourceType(), wyMusicName, ServiceConfigure.WYMUSIC.pkgName() + "." + ServiceConfigure.WYMUSIC.serviceName(), UserHandleInfo.passenger_screen.getUserHandle()));
        configs.add(new RemoteConfig(ServiceConfigure.WYMUSIC.sourceType(), wyMusicName, ServiceConfigure.WYMUSIC.pkgName() + "." + ServiceConfigure.WYMUSIC.serviceName(), UserHandleInfo.ceil_screen.getUserHandle()));
        ...
        configs.add(new RemoteConfig(ServiceConfigure.BTMUSIC.sourceType(), btMusicName, ServiceConfigure.BTMUSIC.pkgName() + "." + ServiceConfigure.BTMUSIC.serviceName(), UserHandleInfo.central_screen.getUserHandle()));
        ...
        configs.add(new RemoteConfig(ServiceConfigure.USBMUSIC.sourceType(), usbMusicName, ServiceConfigure.USBMUSIC.pkgName() + "." + ServiceConfigure.USBMUSIC.serviceName(), UserHandleInfo.central_screen.getUserHandle()));
    ...
    public void initUserHandle(UserHandle userHandle, int displayId) {
        ...
        cmdTarget.setUserId(userHandle.hashCode());
        LogUtils.i(TAG,"initUserHandle: "+userHandle+";displayId:"+displayId);
    }
    ...
    public int switchPage(boolean isOpen,int pageId){
        ...
        if(isOpen){
            ...
            if(DeviceHolder.INS().getDevices().getVoiceCarSignal().isParkingLimitation() && displayId == UserHandleInfo.central_screen.getDisplayId()){
    ...
    @Override
    public boolean isplayingBySource(String source,UserHandle user) {
        boolean ret = false;
        SyncResponse<Integer> command = musicApi.getPlayState(MediaConstant.SOURCE_TYPE,source, user);
        if (command != null && command.getCode() == 0) {
            ret = command.getData() == 1 || command.getData() == 2;
        }
        LogUtils.d(TAG, "isPlaying: "+ret+";source:"+source+" ;userhandle :"+ user);
        return ret;
    }
    ...
    public int pre(String source) {
        ...
        if(TextUtils.isEmpty(source)){
            command = musicApi.prev(MediaConstant.SOURCE_TYPE,cmdTarget);
        } else {
            command = musicApi.prev(MediaConstant.SOURCE_TYPE, source,userHandle);
        }
    ...
    public int next(String source) {
        SyncResponse<Object> command;
        if(TextUtils.isEmpty(source)){
            command = musicApi.next(MediaConstant.SOURCE_TYPE,cmdTarget);
        } else {
            command = musicApi.next(MediaConstant.SOURCE_TYPE,source, userHandle);
        }
    ...
    public int play(String source) {
        ...
        SyncResponse<Object> command;
        if (TextUtils.isEmpty(source)) {
            command = musicApi.play(MediaConstant.SOURCE_TYPE,cmdTarget);
        } else {
            command = musicApi.playSource(MediaConstant.SOURCE_TYPE,source, userHandle);
        }
    ...
    public int stop(String source) {
        SyncResponse<Object> command;
        if(TextUtils.isEmpty(source)){
            command = musicApi.pause(MediaConstant.SOURCE_TYPE,userHandle);
        } else {
            command = musicApi.pause(MediaConstant.SOURCE_TYPE,source,userHandle);
        }
    ...
    public MediaMusicInfo getMediaInfo(String source){
        ...
        if (mediaInfo != null) {
            if (mediaInfo.getCode() == Code.SUCESS.code()) {
                ...
                media.setDuration(mediaInfo.getData().getDuration());
                media.setPlayType(mediaInfo.getData().getPlayType());
    ...
    public void registerPlayStateChanged(){
        ...
        musicApi.onPlayStateChanged(MediaConstant.SOURCE_TYPE, UserHandleInfo.passenger_screen.getUserHandle()).subscribe(notify -> {
            if (MediaSource.wy_music.getName().equals(notify.getSourceType())) {
                if (notify.getPlayStatus() == 2) {
                    LogUtils.d(TAG, "passenger_screen wy is playing");
                    SPUtil.putBoolean(context, MediaConstant.IS_WY_PLAY, true);
                }
            } else if (MediaSource.qq_music.getName().equals(notify.getSourceType())) {
                if (notify.getPlayStatus() == 2) {
                    LogUtils.d(TAG, "passenger_screen qq is playing");
                    SPUtil.putBoolean(context, MediaConstant.IS_WY_PLAY, false);
                }
            }
        });

        musicApi.onPlayStateChanged(MediaConstant.SOURCE_TYPE, UserHandleInfo.ceil_screen.getUserHandle()).subscribe(notify -> {
            if (MediaSource.wy_music.getName().equals(notify.getSourceType())) {
                if (notify.getPlayStatus() == 2) {
                    LogUtils.d(TAG, "ceil_screen wy is playing");
                    SPUtil.putBoolean(context, MediaConstant.IS_WY_PLAY, true);
                }
            } else if (MediaSource.qq_music.getName().equals(notify.getSourceType())) {
                if (notify.getPlayStatus() == 2) {
                    LogUtils.d(TAG, "ceil_screen qq is playing");
                    SPUtil.putBoolean(context, MediaConstant.IS_WY_PLAY, false);
                }
            }
        });
            ...
            public void onSuccess(VoiceSearchSongInfo response) {
                ...
                if(response != null){
                    ...
                    if(voiceMusicSongInfoList != null && !voiceMusicSongInfoList.isEmpty()){
                        ...
                        showMusicCard(voiceMusicSongInfoList,MultiMusicInfo.SourceType.WY_MUSIC);
        ...
        musicApi.qqSearch(MediaConstant.TYPE_QQ, request, userHandle).subscribe(new IpcObserverResponse<List<VoiceSongInfo>>(){
            ...
            public void onSuccess(List<VoiceSongInfo> response) {
                ...
                if (voiceMusicSongInfoList  != null && !voiceMusicSongInfoList .isEmpty()) {
                    ...
                    showMusicCard(voiceMusicSongInfoList,MultiMusicInfo.SourceType.QQ_MUSIC);
                    onResult.onSuccess(voiceMusicSongInfoList.get(0));
                } else {
                    onResult.onSuccess(null);
                }
    ...
    public void playSongsByMediaId(List<VoiceMusicSongInfo> voiceSongInfos, int voiceSongNum, int pos, boolean isAppendSong, String sourceType) {
        playSongsByMediaId(voiceSongInfos, voiceSongNum, pos, isAppendSong, sourceType, userHandle);
    }

    public void playSongsByMediaId(List<VoiceMusicSongInfo> voiceMusicSongInfoList, int voiceSongNum,int pos, boolean isAppendSong, String sourceType,UserHandle user) {
        if (voiceMusicSongInfoList != null && !voiceMusicSongInfoList.isEmpty()) {
            ...
            SyncResponse<Object> command = musicApi.playSong(MediaConstant.SOURCE_TYPE, voicePlayRequest, user);
    ...
    private static boolean isShowMusicCard = false;
    public void setCardInfo(String sessionid, String reqid,String mAgentIdentifier, int soundLocation) {
        this.sessionid = sessionid;
        this.reqid = reqid;
        this.mAgentIdentifier = mAgentIdentifier;
        this.soundLocation = soundLocation;
    }

    private List<VoiceMusicSongInfo> voiceMusicSongInfoList = new ArrayList<>();
    ...
    public void setMusicList(List<VoiceMusicSongInfo> voiceSongInfoList){
        this.voiceMusicSongInfoList = voiceSongInfoList;
    }
    public List<VoiceMusicSongInfo> getMusicList(){
        return voiceMusicSongInfoList;
    }

    public void showMusicCard(List<VoiceMusicSongInfo> voiceSongInfos,int sourceType) {
        setMusicList(voiceSongInfos);
        List<MultiMusicInfo> multiMusicInfos = new ArrayList<>();
        for (int i = 0;i < voiceSongInfos.size();i++){
            if(i >= 24) {
                break;
            }
            VoiceMusicSongInfo voiceSongInfo = voiceSongInfos.get(i);
            MultiMusicInfo multiMusicInfo = new MultiMusicInfo();
            multiMusicInfo.setName(voiceSongInfo.getMediaName());
            multiMusicInfo.setArtist(voiceSongInfo.getArtist());
            multiMusicInfo.setAlbum(voiceSongInfo.getAlbumName());
            multiMusicInfo.setImgUrl(voiceSongInfo.getCover());
            multiMusicInfo.setSourceType(sourceType);
            multiMusicInfo.setVip(voiceSongInfo.isAudition());
            multiMusicInfos.add(multiMusicInfo);
        }

        HashMap<String, Object> map = new HashMap<>();
        map.put("scene", "State.MUSIC_OPTIONS");
        map.put("sessionId", sessionid);
        map.put("requestId", reqid);
        map.put("musicList", multiMusicInfos);
        UIMgr.INSTANCE.showCard(UiConstant.CardType.MUSIC_CARD, map, sessionid, mAgentIdentifier,
                soundLocation);
    ...
    private static final UICardListener uiCardListener = new UICardListener() {
        ...
        public void onCardItemClick(int position, int itemType,int screenType) {
            ...
            if (!DeviceHolder.INS().getDevices().getMediaPage().isMusicType(itemType)) {
                return;
            }

            int currentPage = DeviceHolder.INS().getDevices().getMediaPage().getCurrentPage(screenType).getPosition();
            int maxCount = DeviceHolder.INS().getDevices().getMediaPage().getCurrentPage(screenType).getMaxItemCount();
            ...
            list.add(DeviceHolder.INS().getDevices().getMediaCenter().getVoyahMusicControl().getMusicList().get(pos));
            ...
            DeviceHolder.INS().getDevices().getMediaCenter().getVoyahMusicControl().setMusicList(null);
    ...
    public boolean isShowMusicCard(){
        return isShowMusicCard;
    }
