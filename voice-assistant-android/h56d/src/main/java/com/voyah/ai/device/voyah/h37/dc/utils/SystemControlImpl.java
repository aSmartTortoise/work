...
public class SystemControlImpl implements SystemControlInterface {
    ...
    public boolean isCurVolumeFeatureMode(String switch_mode) {
        if (isHighLevelCar()) {
            ...
            //N2模式调节从0开始，所以默认值=0
            int mode = Settings.Global.getInt(Utils.getApp().getContentResolver(), "key_characteristic_sound", 0);
    ...
    public void setVolumeFeatureMode(String switch_mode) {
        if (isHighLevelCar()) {
            ...
            CarPropUtils.getInstance().setIntProp(H56D.IVI_infoSet1_IVI_AI_MODESET, mode);
            ...
            if (mode == 5) {
                ...
                Settings.Global.putInt(Utils.getApp().getContentResolver(), "key_characteristic_sound", mode);
                Utils.getApp().getContentResolver().notifyChange(Settings.Global.getUriFor("key_characteristic_sound"), null);
    ...
    public String changeVolumeFeatureMode() {
        ...
        int mode;
        if (isHighLevelCar) {
            mode = Settings.Global.getInt(Utils.getApp().getContentResolver(), "key_characteristic_sound", 1);
            nextMode = mode == 8 ? 1 : mode + 1;
        } else {
            mode = Settings.Global.getInt(Utils.getApp().getContentResolver(), "key_characteristic_sound", 0);
            int musicStyleSwitch = Settings.Global.getInt(Utils.getApp().getContentResolver(), "MUSIC_STYLE_SWITCH", 0);
            if (musicStyleSwitch == 1) {
                //AI之后是均衡器
                nextMode = 4;
        ...
        if (isHighLevelCar) {
            //N3N4配置
            CarPropUtils.getInstance().setIntProp(H56D.IVI_infoSet1_IVI_AI_MODESET, nextMode);
    ...
    public boolean isCurVolumeFocusMode(HashMap<String, Object> map) {
        ...
        int modeValue = Settings.Global.getInt(Utils.getApp().getContentResolver(), "key_focus_sound", 1);
    ...
    public void changeVolumeFocusMode(HashMap<String, Object> map) {
        int nextMode = 1;
        int mode = Settings.Global.getInt(Utils.getApp().getContentResolver(), "key_focus_sound", 1);
        if (mode == 1) {
            nextMode = 5;
        } else if (mode == 2) {
            nextMode = 3;
