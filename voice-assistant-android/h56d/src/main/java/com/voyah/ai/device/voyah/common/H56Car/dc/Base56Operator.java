...
public class Base56Operator extends BaseOperator {
    ...
    public int getBaseIntProp(String key, int area) {
        ...
        switch (key) {
            case CarSettingSignal.CARSET_DRIVING_MODE:
                return DrivingHelper.getDrivingMode();
            ...
            case CommonSignal.COMMON_360_STATE:
                return CarPropUtils.getInstance().getIntProp(H56D.APA_avmState_APA_AVMSTS);
    ...
    public void setBaseIntProp(String key, int area, int value) {
        switch (key) {
            ...
            case CommonSignal.COMMON_DISMISS_DIALOG:
                if (value == DeviceHolder.INS().getDevices().getSystem().getScreen().getPassengerScreenDisplayId()) {
                    DeviceHolder.INS().getDevices().getSetting().exec(SettingConstants.NEW_ACTION_hideDialog1);
                } else {
                    DeviceHolder.INS().getDevices().getSetting().exec(SettingConstants.NEW_ACTION_hideDialog2);
                }
                break;
            case CommonSignal.COMMON_OPEN_BT:
                if (value == DeviceHolder.INS().getDevices().getSystem().getScreen().getPassengerScreenDisplayId()) {
                    DeviceHolder.INS().getDevices().getSetting().exec(SettingConstants.NEW_ACTION_btList1);
                } else {
                    DeviceHolder.INS().getDevices().getSetting().exec(SettingConstants.NEW_ACTION_btList2);
                }
                break;
            case CommonSignal.COMMON_OPEN_AP:
                if (value == DeviceHolder.INS().getDevices().getSystem().getScreen().getPassengerScreenDisplayId()) {
                    DeviceHolder.INS().getDevices().getSetting().exec(SettingConstants.NEW_ACTION_hotspot1);
                } else {
                    DeviceHolder.INS().getDevices().getSetting().exec(SettingConstants.NEW_ACTION_hotspot2);
                }
                break;
            case CommonSignal.COMMON_OPEN_WIFI:
                if (value == DeviceHolder.INS().getDevices().getSystem().getScreen().getPassengerScreenDisplayId()) {
                    DeviceHolder.INS().getDevices().getSetting().exec(SettingConstants.NEW_ACTION_wifiList1);
                } else {
                    DeviceHolder.INS().getDevices().getSetting().exec(SettingConstants.NEW_ACTION_wifiList2);
                }
                break;
    ...
    public void setBaseBooleanProp(String key, int area, boolean value) {
        if (key.equalsIgnoreCase(CommonSignal.COMMON_PARK_APP_STATE)) {
            CarPropUtils.getInstance().setIntProp(H56D.IVI_AVMSet_IVI_VOICE_AVMONOFFREQ, value ? 1 : 2);
        } else {
