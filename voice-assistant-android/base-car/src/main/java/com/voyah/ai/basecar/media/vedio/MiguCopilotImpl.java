...
public enum MiguCopilotImpl implements MediaInterface {
    INSTANCE;
    private static final String TAG = MiguCopilotImpl.class.getSimpleName();
...
//    public static final String HISTORY_ACTIVITY = "cn.cmvideo.car.setting.ui.UserHistoryActivity";
    ...
    public void init(Context context) {
        LogUtils.d(TAG, "init");
        this.context = MediaHelper.getUserContext(context, APP_NAME, MediaHelper.getUserHandle(DeviceScreenType.PASSENGER_SCREEN));
        ...
        miguService.connectService(MediaHelper.getUserId(DeviceScreenType.PASSENGER_SCREEN));
    }
    ...
    public TTSBean open() {
        LogUtils.d(TAG, "open");
        if (isFront()) {
            return TtsReplyUtils.getTtsBean("1100029", "@{screen_name}", MediaHelper.SCREEN_NAME_PASSENGER, "@{app_name}", "咪咕视频");
        } else {
            ...
            return TtsReplyUtils.getTtsBean("1100030", "@{screen_name}", MediaHelper.SCREEN_NAME_PASSENGER,"@{app_name}", "咪咕视频");
        }
    }
    ...
    public TTSBean close() {
        ...
        if (isFront()) {
            ...
            return TtsReplyUtils.getTtsBean("1100031", "@{screen_name}", MediaHelper.SCREEN_NAME_PASSENGER, "@{app_name}", "咪咕视频");
        } else {
            if (MediaHelper.isMirrorAppFront(APP_NAME)) {
                MediaHelper.backToHome(DeviceScreenType.PASSENGER_SCREEN);
                return TtsReplyUtils.getTtsBean("1100031", "@{screen_name}", MediaHelper.SCREEN_NAME_PASSENGER, "@{app_name}", "咪咕视频");
            } else {
                return TtsReplyUtils.getTtsBean("1100032", "@{screen_name}", MediaHelper.SCREEN_NAME_PASSENGER, "@{app_name}", "咪咕视频");
            }
        }
    }
