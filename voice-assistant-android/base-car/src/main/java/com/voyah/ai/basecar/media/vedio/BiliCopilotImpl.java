...
public enum BiliCopilotImpl implements MediaInterface {
    INSTANCE;
    private static final String TAG = BiliCopilotImpl.class.getSimpleName();
    ...
    public boolean isPlayPage() {
        boolean playPage = PLAY_ACTIVITY.equals(MediaHelper.getTopActivityName(MegaDisplayHelper.getPassengerScreenDisplayId()));
        return playPage;
    }
    ...
    public TTSBean switchApp(boolean isOpen){
        if (isOpen) {
            if(isFront()) {
                return TtsReplyUtils.getTtsBean("1100029", "@{screen_name}", MediaHelper.SCREEN_NAME_PASSENGER,"@{app_name}", "哔哩哔哩");
            }else{
                open();
                return TtsReplyUtils.getTtsBean("1100030", "@{screen_name}", MediaHelper.SCREEN_NAME_PASSENGER,"@{app_name}", "哔哩哔哩");
            }
        } else {
            if(isFront()) {
                close();
                return TtsReplyUtils.getTtsBean("1100031", "@{screen_name}", MediaHelper.SCREEN_NAME_PASSENGER,"@{app_name}", "哔哩哔哩");
            }else{
                if(MediaHelper.isMirrorAppFront(APP_NAME)){
                    MediaHelper.backToHome(DeviceScreenType.PASSENGER_SCREEN);
                    return TtsReplyUtils.getTtsBean("1100031", "@{screen_name}", MediaHelper.SCREEN_NAME_PASSENGER,"@{app_name}", "哔哩哔哩");
                }else {
                    return TtsReplyUtils.getTtsBean("1100032", "@{screen_name}", MediaHelper.SCREEN_NAME_PASSENGER, "@{app_name}", "哔哩哔哩");
                }
            }
        }
    }
...
//                        int closestInt = (int) closest;
    ...
    private void mediaControl(String cmd, String param) {
        BiliBroadcastReceiver.currentScreenType = ScreenType.PASSENGER;
