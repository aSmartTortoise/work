...
public enum TencentVideoCeilingImpl implements MediaInterface {
    INSTANCE;
    private static final String TAG = TencentVideoCeilingImpl.class.getSimpleName();
    ...
    public boolean isFront() {
        boolean foregroundApp = MediaHelper.isAppForeGround(APP_NAME, MegaDisplayHelper.getCeilingScreenDisplayId());
    ...
    public TTSBean launchTencentVideo(boolean isOpen) {
        ...
        if (isOpen) {
            if (isFront()) {
                return TtsReplyUtils.getTtsBean("1100029", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL, "@{app_name}", "腾讯视频");
            } else {
                isOpen(true);
                return TtsReplyUtils.getTtsBean("1100030", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL, "@{app_name}", "腾讯视频");
            }
        } else {
            if (isFront()) {
                isOpen(false);
                return TtsReplyUtils.getTtsBean("1100031", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL, "@{app_name}", "腾讯视频");
            } else {
                if (MediaHelper.isMirrorAppFront(APP_NAME)) {
                    MediaHelper.backToHome(DeviceScreenType.CEIL_SCREEN);
                    return TtsReplyUtils.getTtsBean("1100031", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL, "@{app_name}", "腾讯视频");
                } else {
                    return TtsReplyUtils.getTtsBean("1100032", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL, "@{app_name}", "腾讯视频");
                }
            }
        }
    }
...
//            MediaHelper.closeApp(APP_NAME,DeviceScreenType.CEIL_SCREEN);
    ...
    private void start(String cmd) {
        ...
        intent.putExtra("userId",String.valueOf(MediaHelper.getUserId(DeviceScreenType.CEIL_SCREEN)));
        Context userContext = MediaHelper.getUserContext(context, APP_NAME, MediaHelper.getUserHandle(DeviceScreenType.CEIL_SCREEN));
        userContext.startService(intent);
    }
...
//        return TTSIDConvertHelper.getInstance().getTTSBean("4028803");
    ...
    public void scheme(String cid) {
        ...
        if (!TextUtils.isEmpty(cid)) {
            ...
            activityOptions.setLaunchDisplayId(MediaHelper.getVideoDisplayId(MegaDisplayHelper.getCeilingScreenDisplayId(), APP_NAME));
