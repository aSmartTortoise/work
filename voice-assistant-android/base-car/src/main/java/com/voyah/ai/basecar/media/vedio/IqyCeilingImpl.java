...
public enum IqyCeilingImpl implements MediaInterface {
    INSTANCE;

    private static final String TAG = IqyCeilingImpl.class.getSimpleName();

    private Context context;

    private final int userId = MegaDisplayHelper.getUserHandleByDisplayId(MegaDisplayHelper.getCeilingScreenDisplayId()).hashCode();
    ...
    public boolean isFront() {
        boolean ret = MediaHelper.isAppForeGround(APP_NAME, MegaDisplayHelper.getCeilingScreenDisplayId());
        return ret;
    }

    public boolean isPlayPage() {
        boolean ret = DETAIL_ACTIVITY.equals(MediaHelper.getTopActivityName(MegaDisplayHelper.getCeilingScreenDisplayId()));
        return isFront() && ret;
    }

    public boolean isCollectFront() {
        return COLLECT_ACTIVITY.equals(MediaHelper.getTopActivityName(MegaDisplayHelper.getCeilingScreenDisplayId()));
    }

    public boolean isHistoryFront() {
        return HISTORY_ACTIVITY.equals(MediaHelper.getTopActivityName(MegaDisplayHelper.getCeilingScreenDisplayId()));
    }
    ...
    public TTSBean open(boolean isOpen) {
        ...
        if (isOpen) {
            if (isFront()) {
                return TtsReplyUtils.getTtsBean("1100029", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL,"@{app_name}", "爱奇艺");
            } else {
                ...
                return TtsReplyUtils.getTtsBean("1100030", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL,"@{app_name}", "爱奇艺");
            }
        } else {
            if (isFront()) {
                //使用爱奇艺api会导致userId is 0 binder service connect failed ,link interface is null
//                mediaControl(QYSExecType.CLOSE_APP, null);
                MediaHelper.closeApp(APP_NAME, DeviceScreenType.fromValue(FuncConstants.VALUE_SCREEN_CEIL));
                return TtsReplyUtils.getTtsBean("1100031", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL,"@{app_name}", "爱奇艺");
            } else {
                if (MediaHelper.isMirrorAppFront(APP_NAME)) {
                    MediaHelper.backToHome(DeviceScreenType.CEIL_SCREEN);
                    return TtsReplyUtils.getTtsBean("1100031", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL, "@{app_name}", "爱奇艺");
                } else {
                    return TtsReplyUtils.getTtsBean("1100032", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL, "@{app_name}", "爱奇艺");
                }
            }
        }
    }
    ...
    private void openApp() {
        MediaHelper.openApp(APP_NAME, DeviceScreenType.fromValue(FuncConstants.VALUE_SCREEN_CEIL));
    }
...
//                }
    ...
    public void scheme(String tvId, String channelId, String albumName) {
        int videoDisplayId = MediaHelper.getVideoDisplayId(MegaDisplayHelper.getCeilingScreenDisplayId(), APP_NAME);
        LogUtils.d(TAG, "videoDisplayId = " + videoDisplayId + "---ceil = " + MegaDisplayHelper.getCeilingScreenDisplayId());
        ...
        if (!TextUtils.isEmpty(displayIdStr)) {
            MegaContext.startActivityAsUser(context,intent,activityOptions.toBundle(),MediaHelper.getUserHandle(DeviceScreenType.CEIL_SCREEN));
        } else {
            MegaContext.startActivityAsUser(context,intent,activityOptions.toBundle(),MediaHelper.getUserHandle(DeviceScreenType.CEIL_SCREEN));
        }
    }
    ...
    private void mediaControl(String cmd,Map<String,String> paramMap){
        ...
        int index = Arrays.binarySearch(targetScreen, ceilingScreenDisplayId);
        if (StringUtils.isNotBlank(mirrorPackage) && APP_NAME.equals(mirrorPackage) && (index >= 0 || sourceScreen == ceilingScreenDisplayId)) {
            paramMap.put("display_id", String.valueOf(MirrorServiceManager.INSTANCE.getVirtualDisplayId()));
        } else {
            paramMap.put("display_id", String.valueOf(ceilingScreenDisplayId));
        }
        if (mServiceConnectionStatus) {
            IqyVoiceCtrManager.getInstance().obtainVoiceMessage(userId, cmd, paramMap);
        } else {
