...
public enum BiliCeilingImpl implements MediaInterface {
    INSTANCE;
    private static final String TAG = BiliCeilingImpl.class.getSimpleName();
    ...
    public boolean isPlayPage() {
        boolean playPage = PLAY_ACTIVITY.equals(MediaHelper.getTopActivityName(MegaDisplayHelper.getCeilingScreenDisplayId()));
        return playPage;
    }
    ...
    public TTSBean switchApp(boolean isOpen){
        if (isOpen) {
            if(isFront()) {
                return TtsReplyUtils.getTtsBean("1100029", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL,"@{app_name}", "哔哩哔哩");
            }else{
                open();
                return TtsReplyUtils.getTtsBean("1100030", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL,"@{app_name}", "哔哩哔哩");
            }
        } else {
            if (isFront()) {
                close();
                return TtsReplyUtils.getTtsBean("1100031", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL, "@{app_name}", "哔哩哔哩");
            } else {
                if (MediaHelper.isMirrorAppFront(APP_NAME)) {
                    MediaHelper.backToHome(DeviceScreenType.CEIL_SCREEN);
                    return TtsReplyUtils.getTtsBean("1100031", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL, "@{app_name}", "哔哩哔哩");
                } else {
                    return TtsReplyUtils.getTtsBean("1100032", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL, "@{app_name}", "哔哩哔哩");
                }
            }
        }
    }
    ...
    private void initService(ServiceConnection serviceConnection) {
        ...
        MegaContext.bindServiceAsUser(context, intent, serviceConnection, Context.BIND_AUTO_CREATE, MediaHelper.getUserHandle(DeviceScreenType.CEIL_SCREEN));
    }
    ...
    public TTSBean switchDanmaku(boolean isOpen) {
        ...
        BiliBroadcastReceiver.switchDanmakuOpen = isOpen;
        BiliBroadcastReceiver.actionType = isOpen ? OPEN_DANMAKU : CLOSE_DANMAKU;
            ...
            if (TextUtils.equals(level, "max")) {
                ...
                try {
                    ...
                    if (num > 0) {
                        ...
                        float closest;
                        if (num > 2.0f) {
                            BiliBroadcastReceiver.isOutRangeMaxSpeed = true;
                            closest = 2.0f;
                        } else if (num < 0.5f) {
                            BiliBroadcastReceiver.isOutRangeMinSpeed = true;
                            closest = 0.5f;
                        } else {
                            closest = NumberUtils.findClosestValue(num, targets);
                        }
                        BiliBroadcastReceiver.currentSpeed = closest;
...
//                        int closestInt = (int) closest;
    ...
    public TTSBean switchHistoryUI(boolean isOpen) {
        ...
        String param = "list:";
        if (isOpen) {
            if (StringUtils.isBlank(BiliBroadcastReceiver.actionType)) {
                BiliBroadcastReceiver.actionType = OPEN_HISTORY_UI;
            }
            ...
            BiliBroadcastReceiver.actionType = CLOSE_HISTORY_UI;
    ...
    public TTSBean switchCollectUI(boolean isOpen) {
        ...
        if (isOpen) {
            if (StringUtils.isBlank(BiliBroadcastReceiver.actionType)) {
                BiliBroadcastReceiver.actionType = OPEN_COLLECT_UI;
            }
            ...
            BiliBroadcastReceiver.actionType = CLOSE_COLLECT_UI;
            param += MediaConstant.PageName.VIDEO;
        }
        try {
            if (!isFront()) {
    ...
    public TTSBean playUI(int type) {
        if (type == 1) {
            ...
            BiliBroadcastReceiver.actionType = PLAY_HISTORY;
            switchHistoryUI(true);
        } else {
            BiliBroadcastReceiver.actionType = PLAY_COLLECT;
    ...
    public void playEpisode(int value) {
        ...
        BiliBroadcastReceiver.playEpisodeValue = value;
    ...
    private void mediaControl(String cmd, String param) {
        BiliBroadcastReceiver.currentScreenType = ScreenType.CEILING;
