...
public enum TiktokCeilingImpl implements MediaInterface {
    INSTANCE;
    private static final String TAG = TiktokCeilingImpl.class.getSimpleName();
    ...
    public TTSBean switchApp(boolean isOpen){
        if (isOpen) {
            if(isFront()) {
                return TtsReplyUtils.getTtsBean("1100029", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL,"@{app_name}", "车鱼视听");
            }else{
                open();
                return TtsReplyUtils.getTtsBean("1100030", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL,"@{app_name}", "车鱼视听");
            }
        } else {
            if (isFront()) {
                close();
                return TtsReplyUtils.getTtsBean("1100031", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL, "@{app_name}", "车鱼视听");
            } else {
                if (MediaHelper.isMirrorAppFront(APP_NAME)) {
                    MediaHelper.backToHome(DeviceScreenType.CEIL_SCREEN);
                    return TtsReplyUtils.getTtsBean("1100031", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL, "@{app_name}", "车鱼视听");
                } else {
                    return TtsReplyUtils.getTtsBean("1100032", "@{screen_name}", MediaHelper.SCREEN_NAME_CEIL, "@{app_name}", "车鱼视听");
                }
            }
        }
    }
