...
public enum IqyImpl implements MediaInterface {
    INSTANCE;

    private static final String TAG = IqyImpl.class.getSimpleName();
    ...
    private Map<Integer, Boolean> mConnectionStatusMap;
    ...
    public boolean isFrontByDisplayid(int displayid) {
        boolean ret = MediaHelper.isAppForeGround(APP_NAME, displayid);
        return ret;
    }

    public boolean isPlayPage() {
        boolean ret = DETAIL_ACTIVITY.equals(MediaHelper.getTopActivityName(MegaDisplayHelper.getMainScreenDisplayId()));
        return isFront() && ret;
    }

    public boolean isPlayPage(int displayId) {
        boolean ret = DETAIL_ACTIVITY.equals(MediaHelper.getTopActivityName(displayId));
        return isFront() && ret;
    }
    ...
    public synchronized boolean isPlaying() {
        ...
        if (Boolean.TRUE.equals(mConnectionStatusMap.getOrDefault(currentDisplayId, false))) {
            ret[0] = IqyVoiceCtrManager.getInstance().videoIsPlaying(MediaHelper.getUserIdByDisplayId(currentDisplayId));
        } else {
            IqyVoiceCtrManager.getInstance().setOnServiceConnectListener(MediaHelper.getUserIdByDisplayId(currentDisplayId), new IServiceConnectListener.Stub() {
                @Override
                public void onServiceConnection(boolean b) {
                    LogUtils.d(TAG, "onServiceConnection: " + b);
                    mConnectionStatusMap.put(currentDisplayId,b);
                    if (b) {
                        registerIQysLinkResponse(MediaHelper.getUserIdByDisplayId(currentDisplayId));
                        ret[0] = IqyVoiceCtrManager.getInstance().videoIsPlaying(MediaHelper.getUserIdByDisplayId(currentDisplayId));
                        LogUtils.d(TAG, "onServiceConnection1: " + ret[0]);
                    }
    ...
    public synchronized boolean isPlaying(int displayId) {
        final boolean[] ret = new boolean[1];
        if (Boolean.TRUE.equals(mConnectionStatusMap.getOrDefault(displayId, false))) {
            ret[0] = IqyVoiceCtrManager.getInstance().videoIsPlaying(MediaHelper.getUserIdByDisplayId(displayId));
        } else {
            IqyVoiceCtrManager.getInstance().setOnServiceConnectListener(MediaHelper.getUserIdByDisplayId(displayId), new IServiceConnectListener.Stub() {
                @Override
                public void onServiceConnection(boolean b) {
                    LogUtils.d(TAG, "onServiceConnection: " + b + ", displayId = " + displayId);
                    if (b) {
                        if (Boolean.FALSE.equals(mConnectionStatusMap.getOrDefault(displayId, false))) {
                            registerIQysLinkResponse(MediaHelper.getUserIdByDisplayId(displayId));
                        }
                        ret[0] = IqyVoiceCtrManager.getInstance().videoIsPlaying(MediaHelper.getUserIdByDisplayId(displayId));
                        LogUtils.d(TAG, "onServiceConnection1: " + ret[0]);
                    }
                    mConnectionStatusMap.put(displayId, b);
                    countDownLatch.countDown();
                }
            });
            try {
    ...
    private void registerIQysLinkResponse(int userId) {
    ...
    public TTSBean open(boolean isOpen) {
        ...
        if (isOpen) {
            if (isFront()) {
                return MediaTtsManager.getInstance().getAlreadyOpenAppTts(MediaTtsManager.APP_NAME_IQY,MediaHelper.SCREEN_NAME_CENTRAL);
            } else {
                ...
                return MediaTtsManager.getInstance().getOpenAppTts(MediaTtsManager.APP_NAME_IQY,MediaHelper.SCREEN_NAME_CENTRAL);
            }
        } else {
            if (isFront()) {
                ...
                return MediaTtsManager.getInstance().getCloseAppTts(MediaTtsManager.APP_NAME_IQY,MediaHelper.SCREEN_NAME_CENTRAL);
            } else {
                if (MediaHelper.isMirrorAppFront(APP_NAME)) {
                    MediaHelper.backToHome(DeviceScreenType.CENTRAL_SCREEN);
                    return MediaTtsManager.getInstance().getCloseAppTts(MediaTtsManager.APP_NAME_IQY,MediaHelper.SCREEN_NAME_CENTRAL);
                } else {
                    return MediaTtsManager.getInstance().getAlreadyCloseAppTts(MediaTtsManager.APP_NAME_IQY,MediaHelper.SCREEN_NAME_CENTRAL);
                }
            }
        }
    }
    ...
    public void init(Context context) {
        ...
        mConnectionStatusMap = new HashMap<>();
    }
    ...
    public TTSBean switchHistoryUI(boolean isOpen) {
        ...
        if (MediaHelper.isSafeLimitation() && isOpen) {
...
//            }
    ...
    public void scheme(String tvId, String channelId, String albumName) {
        int videoDisplayId = MediaHelper.getVideoDisplayId(VideoControlCenter.getInstance().getCurrentDisplayId(), APP_NAME);
        LogUtils.d(TAG, "videoDisplayId = " + videoDisplayId + "---main = " + MegaDisplayHelper.getMainScreenDisplayId());
        ...
        if (!TextUtils.isEmpty(displayIdStr)) {
            ...
            options.setLaunchDisplayId(videoDisplayId);
    ...
    private void mediaControl(String cmd,Map<String,String> paramMap){
        ...
        int index = Arrays.binarySearch(targetScreen, mainScreenDisplayId);
        if (StringUtils.isNotBlank(mirrorPackage) && APP_NAME.equals(mirrorPackage) && (index >= 0 || sourceScreen == mainScreenDisplayId)) {
            paramMap.put("display_id", String.valueOf(MirrorServiceManager.INSTANCE.getVirtualDisplayId()));
        } else {
            paramMap.put("display_id", String.valueOf(VideoControlCenter.getInstance().getCurrentDisplayId()));
        }
        sendCmd(cmd, paramMap);
    }

    private void sendCmd(String cmd, Map<String, String> paramMap) {
        int displayId = VideoControlCenter.getInstance().getCurrentDisplayId();
        if (Boolean.TRUE.equals(mConnectionStatusMap.getOrDefault(displayId, false))) {
            IqyVoiceCtrManager.getInstance().obtainVoiceMessage(MediaHelper.getUserIdByDisplayId(displayId), cmd, paramMap);
        } else {
            IqyVoiceCtrManager.getInstance().setOnServiceConnectListener(MediaHelper.getUserIdByDisplayId(displayId), new IServiceConnectListener.Stub() {
                @Override
                public void onServiceConnection(boolean b) {
