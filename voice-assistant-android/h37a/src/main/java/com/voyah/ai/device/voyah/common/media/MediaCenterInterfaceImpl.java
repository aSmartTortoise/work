...
public class MediaCenterInterfaceImpl implements MediaCenterInterface {
    ...
    private void initUserHandle(UserHandle userHandle,int displayId){
        ...
        VideoControlCenter.getInstance().setCurrentDisplayId(MediaHelper.getMainScreenDisplayId());
    }
...
//                    }
                    ...
                    if ("music".equals(mediaType)) {
                        ...
                        if (VoyahMusicImpl.INSTANCE.isPlaying()) {
            ...
            if(VoyahMusicImpl.INSTANCE.isPlaying() || voyahMusicControlInterface.isVoyahMusicFront()){
    ...
    public Map<String, String> uploadPlayStatus() {
        ...
        if(voyahMusicControlInterface.isVoyahMusicFront()){
            foregroundMediaSource = VoyahMusicImpl.INSTANCE.getMediaUiResource();
        } else if (IqyImpl.INSTANCE.isFront()) {
            foregroundMediaSource = MediaSource.iqy_video.getType();
        } else if (TencentVideoImpl.INSTANCE.isFront()) {
            foregroundMediaSource = MediaSource.tencent_video.getType();
        } else if (MiguImpl.INSTANCE.isFront()) {
            foregroundMediaSource = MediaSource.migu_video.getType();
        } else if (ThunderKtvImpl.INSTANCE.isFront()) {
            foregroundMediaSource = MediaSource.ls_ktv.getType();
        } else if (TiktokImpl.INSTANCE.isFront()) {
            foregroundMediaSource = MediaSource.tiktok_video.getType();
        } else if (UsbVideoImpl.INSTANCE.isFront()) {
            foregroundMediaSource = MediaSource.usb_video.getType();
        } else if (BiliImpl.INSTANCE.isFront()) {
            foregroundMediaSource = MediaSource.bili_video.getType();
        } else {
            foregroundMediaSource = "";
        }

        Map<String, String> map = new HashMap<>();
        map.put("mediaSource", mediaSource);
        map.put("foregroundMediaSource",foregroundMediaSource);
    ...
    public void setTiktokPlayStatus(boolean isPlaying) {
        TiktokImpl.INSTANCE.setPlaying(isPlaying);
    }

    @Override
    public void gestureTencent() {
        if (MediaDeviceService.IS_PLAYING) {
            LogUtils.d(TAG, "isRealPlaying");
            if (TencentVideoImpl.INSTANCE.isPlayPage()) {
                TencentVideoImpl.INSTANCE.stop(false);
            }
        } else {
            LogUtils.d(TAG, "is not RealPlaying");
            if (TencentVideoImpl.INSTANCE.isPlayPage()) {
                TencentVideoImpl.INSTANCE.play();
            }
        }
    }
