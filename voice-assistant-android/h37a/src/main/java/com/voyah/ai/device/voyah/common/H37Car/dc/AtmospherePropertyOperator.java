...
import com.voyah.ai.device.voyah.common.H37Car.AtmosphereHelper;
...
public class AtmospherePropertyOperator extends Base37Operator implements IDeviceRegister {
    ...
    public int getBaseIntProp(String key, int area) {
        switch (key) {
            ...
            case AtmosphereSignal.ATMO_STATIC_MULTI_COLOR:
                return carPropHelper.getIntProp(Signal.ID_AMB_STATIC_MULTIPLE_COLOR);
            case AtmosphereSignal.ATMO_DYNAMIC_SINGLE_COLOR:
                return carPropHelper.getIntProp(Signal.ID_SINGLE_COLOR_MUSIC_RHYTHM);
            ...
            case AtmosphereSignal.ATMO_DYNAMIC_BRIGHTNESS:
                return carPropHelper.getIntProp(Signal.ID_AMB_BRIGHT_DYNAMIC);
    ...
    public boolean getBaseBooleanProp(String key, int area) {
        ...
        } else if (AtmosphereSignal.ATMO_CENTER_SPEAKER_SWITCH_STATE.equals(key)) {
            return AtmosphereHelper.getCenterSpeakerSwitchState();
    ...
    public void setBaseBooleanProp(String key, int area, boolean value) {
        if (AtmosphereSignal.ATMO_EFFECT_MODE.equals(key)) {
            ...
            AtmosphereHelper.setCenterSpeakerSwitchState(value);
        } else if (AtmosphereSignal.ATMO_ATMOSPHERE_PAGE_STATE.equalsIgnoreCase(key)) {
    ...
    public void registerDevice() {
        String atmoIsFollowMusicSwitch;
        String carType = CarServicePropUtils.getInstance().getCarType();
        if (carType.equalsIgnoreCase("H37A")) {
            atmoIsFollowMusicSwitch = "-1";
        } else {
            atmoIsFollowMusicSwitch = "1";
        }
        // 音乐律动是否是开关，37A是模式切换 37B是开关 56C没有音乐律动 56D是模式切换
        FunctionDeviceMappingManager.getInstance().registerFunctionPosition(AtmosphereSignal.ATMO_IS_FOLLOW_MUSIC_SWITCH, FunctionalPositionBean.FUNCTIONAL_POSITION_TYPE_FUNCTIONAL, atmoIsFollowMusicSwitch);
    }
}