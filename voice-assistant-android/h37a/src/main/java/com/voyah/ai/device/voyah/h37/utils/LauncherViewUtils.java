...
//        LogUtils.d(TAG, "isNegativeScreenOpen:" + isNegativeScreenOpen);
    ...
    //应用中心弹窗
    public static boolean isAllAppsOpen() {
...
//        LogUtils.d(TAG, "isAirOpen:" + isAirOpen);
    ...
    //座椅弹窗
    public static boolean isSeatPageOpen() {
        //todo:承举、俞维
        boolean isSeatPageOpen = SeatServiceImpl.getInstance(Utils.getApp()).isCurrentState("ACTION_GET_SEAT_PANEL_STATE");
//        LogUtils.d(TAG, "isSeatPageOpen:" + isSeatPageOpen);
    ...
    public static boolean isParkOpen() {
        int intProp = CarPropUtils.getInstance().getIntProp(Apa.ID_APA_PAS_FUNCMODE);
        return intProp != 0;
    }
    ...
    public static boolean isScreenPopWindowShowTop(boolean isShowLog, String assignScreen) {
        //负一屏
        boolean isNegativeScreenShow = SystemUiUtils.getInstance().isNegativeScreenOpen();
        //应用中心
        boolean isAllAppsShow = isAllAppsOpen();
        //空调
        boolean isAirPageOpen = SystemUiUtils.getInstance().isAirPageOpen(assignScreen);
        //座椅
        boolean isSeatPageOpen = isSeatPageOpen();
        //泊车辅助
        boolean isParkingOpen = isParkingOpen();
        //屏幕清洁模式
    ...
    public static void closeScreenCentral(String assignScreen, Object saveTag) {
        ...
        try {
            //负一屏
            if (!tags.contains(NEGATIVE_SCREEN_TAG) && SystemUiUtils.getInstance().isNegativeScreenOpen())
                SystemUiUtils.getInstance().getNegativeScreenState(false);
            //应用中心
            if (!tags.contains(ALL_APPS_TAG) && isAllAppsOpen())
            ...
            if (!tags.contains(AIR_PAGE_TAG) && SystemUiUtils.getInstance().isAirPageOpen(assignScreen))
                SystemUiUtils.getInstance().closeAirPage();
            //座椅
            if (!tags.contains(SEAT_PAGE) && isSeatPageOpen())
                SystemUiUtils.getInstance().closeSeatPage();
            //泊车辅助
            if (!tags.contains(PARK_PAGE) && isParkingOpen())
                closeParking();
            //屏幕清洁模式
            ...
            //关弹窗的接口可能会崩溃，这里catch住保证至少不影响外部打开页面的执行
            e.printStackTrace();
        }
    }
    ...
    public static String getAssignScreen(HashMap<String, Object> map) {
        ...
        if (LOCATION_LIST.contains(screen)) {
            ...
            if (FIRST_SCREEN.contains(screen))
                assignScreenName = FuncConstants.VALUE_SCREEN_CENTRAL;
            else if (StringUtils.equals(screen, SCREEN_LIST.get(4)))
                assignScreenName = FuncConstants.VALUE_SCREEN_CEIL;
        }
